...
Project.prototype.template = Promise.method(function() {
	...
	if (!_this.isTemplate()) {
		throw new Error( 'The template ' + name + ' does not exist, please create it first using: s3f new ' + name )
	}
...
Project.prototype.isProject = function () {
	var _this     = this;
	var isProject = jetpack.exists( jetpack.path( _this.project.path, '.s3f' ) );
